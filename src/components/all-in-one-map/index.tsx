...
export const AllInOneMap: FunctionComponent<AllInOneMapProps> = (props) => {
  ...
  useEffect(() => {
    if (props.zoom) {
      dispatch({ type: 'SET_DEFAULT_ZOOM', payload: props.zoom })
    }
    if (props.center) {
      dispatch({ type: 'SET_DEFAULT_CENTER', payload: props.center })
    }
    if (!state.google) {
      loadGmApi()
    }
  ...
  const hideStyle: CSSProperties = { position: 'absolute', width: '100%', height: '100%', display: 'none' }
  ...
  if (google) {
    ...
    return <Empty
      style={{ margin: 'auto' }}
      description={
        `No ${state.mapProvider} Api loaded, please check the Internet connection`
      } />
  }
}
